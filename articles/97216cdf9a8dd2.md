...
### 1.1 画像ファイルは 3.75MB 制限。気づきにくい同期失敗の温床
...
### 1.2 マルチモーダル KB のファイル数上限と FM パーサーの観測上の制約
...
### 1.3 1 つの KB に紐づけられるデータソースは 5 つまで。データソース内のファイル数にも上限あり
...
### 1.4 自動作成 IAM ロール / ポリシーは「コンソール用途専用」と心得る
...
### 2.1 `.metadata.json` のキーワードは本文中にも必要 (暗黙的フィルタリング使用時)
...
### 4.1 プライベート KB は OpenSearch Dashboards でインデックスを見られない
...
### 4.2 Bedrock の VPC エンドポイントは 5 種類。役割を取り違えるな
...
### 4.3 ガードレールの言語サポートは「フィルタごと」に異なる。日本語対応は 1 つではない
...
### 5.1 LLM 呼び出し単位のコスト配分: アプリケーション推論プロファイル
...
### 5.2 OpenSearch Serverless Collections のタグはコスト配分に出ない
